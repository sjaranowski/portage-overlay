...
DRIVERS_URI="mirror://ubuntu/pool/restricted/f/fglrx-installer/fglrx-installer_15.200.1.orig.tar.gz"
...
pkg_pretend() {
	...
	if [[ -n $(grep -i "EXPORT_SYMBOL_GPL(cpu_tlbstate);" /usr/src/linux/arch/x86/mm/init.c) ]]; then   	
	die "The kernel source code file /usr/src/linux/arch/x86/mm/init.c contains
EXPORT_SYMBOL_GPL(cpu_tlbstate). This will prevent MODPOST installing the 
fglrx module. You, as a user, can change EXPORT_SYMBOL_GPL(cpu_tlbstate) to
EXPORT_SYMBOL(cpu_tlbstate) by running:
sed -i 's/EXPORT_SYMBOL_GPL(cpu_tlbstate)/EXPORT_SYMBOL(cpu_tlbstate)/g'
/usr/src/linux/arch/x86/mm/init.c' in a terminal or by using a text editor.
A similar problem may exist with EXPORT_SYMBOL_GPL(pci_ignore_hotplug); in
/usr/src/linux/drivers/pci/pci.c) . You may also run:
sed -i 's/EXPORT_SYMBOL_GPL(pci_ignore_hotplug)/EXPORT_SYMBOL(pci_ignore_hotplug)/g'
/usr/src/linux/drivers/pci/pci.c
After the edit; recompile/reinstall the kernel and emerge ati-drivers."
	exit
	fi

	if [[ -n $(grep -i "EXPORT_SYMBOL_GPL(pci_ignore_hotplug);" /usr/src/linux/drivers/pci/pci.c) ]]; then   	
	die "The kernel source code file /usr/src/linux/drivers/pci/pci.c contains
EXPORT_SYMBOL_GPL(pci_ignore_hotplug). This will prevent MODPOST installing the 
fglrx module. You, as a user, can change EXPORT_SYMBOL_GPL(pci_ignore_hotplug) to
EXPORT_SYMBOL(pci_ignore_hotplug) by running:
sed -i 's/EXPORT_SYMBOL_GPL(pci_ignore_hotplug)/EXPORT_SYMBOL(pci_ignore_hotplug)/g' 
/usr/src/linux/drivers/pci/pci.c in a terminal or by using a text editor.
After the edit; recompile/reinstall the kernel and emerge ati-drivers."
	exit
	fi
...
src_prepare() {
	...
	# Compile fix, To use AMD Catalyst™ 15.7 Proprietary Linux Graphics Driver and kernel-4 and newer  
	epatch "${FILESDIR}/15.7-r2.patch"
